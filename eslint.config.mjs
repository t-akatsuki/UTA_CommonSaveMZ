...
import pluginStylisticsJs from "@stylistic/eslint-plugin-js";
 ...
 * @type {string}
 */
const TARGET_EDITION = RPG_MAKER_EDITION.MZ;
 ...
 * @return {Object} languageOptions設定連想配列。
 */
function getLanguageOptions() {
  let config = {
    // ブラウザ向けスクリプトとして読み込まれる
    // moduleで読み込まれない
    "sourceType": "script",
    // 許容するグローバルオブジェクトの定義
    // 対象に対応したものをセットする
  ...
  // 対象によって設定値が異なる要素を格納していく
  switch (TARGET_EDITION) {
    /**
     * RPGツクールMVの場合
     */
    case RPG_MAKER_EDITION.MV:
      // languageOptions.ecmaVersion
      // RPGツクールMVの初期版の後方互換性を担保する場合はES5
      // 最新版のみを対象とする場合はES6(ES2015)で良い
      config.ecmaVersion = 5;
     ...
     * RPGツクールMZの場合
     */
    case RPG_MAKER_EDITION.MZ:
      // languageOptions.ecmaVersion
      // RPGツクールMZではES6(ES2015)前提で良い
      config.ecmaVersion = 2015;
     ...
     */
    default:
      throw new Error(`'TARGET_EDITION' is invalid.`);
  }
...
export default [
  ...
  {
    "plugins": {
      "@stylistic/js": pluginStylisticsJs,
    },
    "files": [
      "src/**/*.js",
    ],
  ...
  // プラグインのlint対象に除外設定を適用する為、
  // 除外項目を別ブロックで定義する必要がある
  {
    "ignores": [
      "node_modules/**",
      "_old/**",
